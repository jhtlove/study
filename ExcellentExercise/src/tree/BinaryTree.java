...
class NodeStack
{
	...
	public static void pushStack(LinkNode n)
	{
			n.next = head;
			head = n;
	}
	
	public static LinkNode popStack()
	{
		if(head == null)
		{
			return null;
...
public class BinaryTree
{
	...
	// 求二叉树结点个数
	public static int treeSize(TreeNode t)
	{
		if (t == null)
		{
			return 0;
		}
		return 1 + treeSize(t.lchirld) + treeSize(t.rchirld);
	}
	...
	public static int treeHeight(TreeNode t)
	{
		...
		int i = treeHeight(t.lchirld) + 1;
		int j = treeHeight(t.rchirld) + 1;
		return (i > j) ? i : j;
	}

	//复制一颗二叉树
	public static TreeNode copyTree(TreeNode orignode)
	{
		if(orignode == null)
		{
			return null;
		...
		temp.lchirld = copyTree(orignode.lchirld);
		temp.rchirld = copyTree(orignode.rchirld);
		return temp;
	}
	
	
	//判断两棵树是否相等
	public static boolean equalTree(TreeNode t1,TreeNode t2)
	{
		if(t1==null&&t2==null)
		{
			return true;
		}
		if(t1!=null&&t2!=null&&t1.data==t2.data&&equalTree(t1.lchirld,t2.lchirld)&&equalTree(t1.rchirld,t2.rchirld))
		{
			return true;
		}
		return false;
	...
	//利用前序遍历，以广义表形式打印二叉树A((B(d)),(c(e))),广义表形式不区分左右子树，只能确定是其子树
	...
	public static void printTree(TreeNode t)
	{
		...
		if(t.lchirld!=null||t.rchirld!=null)
		{
			System.out.print("(");
			if(t.lchirld!=null)
			{
	...
	public static void main(String[] args)
	{
		...
		TreeNode root2 = creatFullBinaryTree(7);
		searchByTier(root2);
		printTree(root2);
	}
